...
function main() {
	const CHAIN_JSON_PATH: string = "./blockchain.json";

	if (!fs.existsSync(CHAIN_JSON_PATH)) {
		let empty_chain_json: { chain: Block[] } = { chain: [] };
		fs.writeFileSync(CHAIN_JSON_PATH, JSON.stringify(empty_chain_json));
	}

	let chain = new BlockChain();
	chain.load(CHAIN_JSON_PATH);
	chain.mineAndPush("test message (encrypted with pk)", "test userdata (encrypted with sk)");
	chain.saveAs(CHAIN_JSON_PATH);
};
main();